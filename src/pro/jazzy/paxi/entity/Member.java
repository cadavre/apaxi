...
public class Member extends OnRoadEventBasicImpl {
    ...
    public float howMuchToPay() {
        ...
        ArrayList<OnRoadEvent> allEvents = PaxiUtility.currentRoute.getRoadEvents();
        float toPay = 0;
        int i = 0; // firstEvent
        ...
        for (int k = i + 1; k <= j; k++) {
            ...
            toPay += calculate(onroadEvent.onWhatDistance() - lastKnownDistance, currentType,
                    membersOnboard);
            if (onroadEvent instanceof Member) {
                membersOnboard++;
            } else if (onroadEvent instanceof MemberOutEvent) {
        ...
        if (this.isOnboard() && lastKnownDistance < PaxiUtility.currentRoute.getCurrentDistance()) {
            toPay += calculate(PaxiUtility.currentRoute.getCurrentDistance(), currentType,
                    membersOnboard);
        }
        return toPay;
    }
